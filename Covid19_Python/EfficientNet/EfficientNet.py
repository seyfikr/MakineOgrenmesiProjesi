...
# EfficientNet için yepyeni bir karargah açıyoruz
save_dir = r"C:\Users\ThinkPad\Desktop\Makine\Covid19-3\EfficientNet_feature_Selection"
...
# ============================================================
# 4. EFFICIENTNET-B0 MİMARİSİ (Özellik Çıkarıcı Modifikasyonu)
# ============================================================
print("\nEfficientNet-B0 Yükleniyor...")

class EfficientNetExtractor(nn.Module):
    def __init__(self):
        super().__init__()
        # PyTorch'un en güncel ve güçlü ağırlıklarını kullanıyoruz
        effnet = models.efficientnet_b0(weights=models.EfficientNet_B0_Weights.DEFAULT)
        # Sadece özellikleri (features) alıyoruz, sınıflandırıcıyı (classifier) atıyoruz
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.pool(x)
        # Çıkan tensörü [batch_size, 1280, 1, 1] halinden [batch_size, 1280] matrisine eziyoruz
        x = torch.flatten(x, 1)
        return x

base_model = EfficientNetExtractor().to(device)
base_model.eval()
...
def extract_all_features(loader):
    print(f"\nTüm Veri Seti İçin Yüksek Çözünürlüklü (299x299) EfficientNet Özellik Çıkarımı Başlıyor...")
    features_list = []
    labels_list = []
...
np.save(os.path.join(save_dir, 'X_features_efficientnet.npy'), X_all)
...
print("==================================================")
print("✔ İŞLEM TAMAM! 1280 Boyutlu EfficientNet özellikleri hazırlandı.")
print("==================================================")