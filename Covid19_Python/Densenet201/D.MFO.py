...
print("==================================================")
print("🦋 MFO (PERVANE-ATEŞ) ÖZELLİK SEÇİMİ BAŞLIYOR 🦋")
print("==================================================")
...
for t in range(max_iter):
    ...
    for i in range(num_moths):
        # Sınır kontrolü (0 ve 1 arasına sıkıştır)
        Moth_Positions[i] = np.clip(Moth_Positions[i], 0, 1)
        Moth_Scores[i] = fitness(Moth_Positions[i])
        
    # 2. Ateşleri (Flames) Güncelle
    if t == 0:
        # İlk turda en iyi pervaneler ateş olur
        sort_indices = np.argsort(Moth_Scores)[::-1] # Büyükten küçüğe
        Flame_Scores = Moth_Scores[sort_indices].copy()
        Flame_Positions = Moth_Positions[sort_indices].copy()
    else:
        # Sonraki turlarda eski ateşler ile yeni pervaneleri birleştir ve en iyilerini seç
