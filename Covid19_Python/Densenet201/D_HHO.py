...
print("==================================================")
print("🦅 HHO (HARRIS HAWKS) ÖZELLİK SEÇİMİ BAŞLIYOR 🦅")
print("==================================================")
...
# ============================================================
# 2. HHO PARAMETRELERİ (Komutuna Göre Ayarlandı)
# ============================================================
num_hawks = 10
max_iter = 10
patience_limit = 4  
...
# Tavşan (Av) - Yani şu ana kadar bulunan en iyi özellik seti
Rabbit_Location = np.zeros(num_features)
Rabbit_Energy_Score = float('-inf')

# Şahinlerin rastgele başlatılması (0 ve 1'lerden oluşan matris)
Hawks_Positions = np.random.randint(2, size=(num_hawks, num_features)).astype(float)

cache = {}

# ============================================================
# 3. FİTNESS (Uygunluk) FONKSİYONU (GWO ile birebir aynı - Adil Yarış)
# ============================================================
def fitness(position):
    # Pozisyonu binary (0 ve 1) hale getirip değerlendir
...
for t in range(max_iter):
    ...
    # 1. Şahinlerin Uygunluğunu Ölç ve Avı (Tavşanı) Bul
    for i in range(num_hawks):
        fit = fitness(Hawks_Positions[i])
        
        # Eğer bir şahin tavşandan daha iyi bir av bulduysa, yeni tavşan o olur
        if fit > Rabbit_Energy_Score:
            Rabbit_Energy_Score = fit
            Rabbit_Location = Hawks_Positions[i].copy()

    # Binary formata çevrilmiş halindeki özellik sayısını bul
    ...
    # --------------------------------------------------------
    E0 = 2 * np.random.rand(num_hawks) - 1  # Başlangıç enerjisi (-1 ile 1 arası)
    E = 2 * E0 * (1 - (t / max_iter))       # Tavşanın kaçış enerjisi (Zamanla azalır)
    
    mean_position = np.mean(Hawks_Positions, axis=0)

    for i in range(num_hawks):
        jump_strength = 2 * (1 - np.random.rand()) # Tavşanın zıplama gücü (J)
        
        if abs(E[i]) >= 1:
            # KEŞİF AŞAMASI (Tavşan enerjik, şahinler dağınık arar)
            q = np.random.rand()
            if q >= 0.5:
            ...
            # SÖMÜRÜ AŞAMASI (Tavşan yoruldu, kuşatma başlıyor)
            r = np.random.rand() # Tavşanın kaçma ihtimali
